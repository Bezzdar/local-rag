...
### Мультиагентный режим (через Ollama)
- Реестр агентов хранится в `agent/registry.json` и переносится вместе с проектом.
- Каждый агент — отдельная роль (например, `copywriter`, `designer`, `chemist`) с собственной функцией и набором tools.
- Подключение выполняется к локальному провайдеру **Ollama** (по умолчанию `http://127.0.0.1:11434`).
- В режиме `agent` backend читает манифест агента, формирует системную «карточку роли» и отправляет запрос в Ollama Chat API (`/api/chat`).
- Для стабильного переноса между машинами держите одинаковые имена моделей в `registry.json` и заранее скачивайте их командой `ollama pull <model>`.
