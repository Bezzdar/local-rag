...
- `pdf_chunk_tester.py` — локальная диагностика качества извлечения/нарезки PDF.
- `Test.bat`, `run.bat`, `setup_and_run.bat`, файлы `*.bat` с именами моделей — вспомогательные скрипты запуска под Windows.
...
Локальные бинарники и DLL для запуска моделей/утилит на Windows (включая server/cli/bench tools).
...
## 5) Что уже оптимизировано в парсинге для качества RAG
