...
## 8.4 Логирование (актуальная схема)

### Файлы логов

Логи создаются в `data/logs/sessions/` и привязаны к **сессии** (= один запуск uvicorn):

| Файл | Содержимое |
|------|-----------|
| `app_<SESSION_ID>.log` | HTTP-запросы/ответы, индексация, ошибки бэкенда |
| `ui_<SESSION_ID>.log` | UI-события: нажатия кнопок, действия пользователя |

`SESSION_ID` = `YYYY-MM-DD_HH-MM` (момент старта бэкенда).

### Ротация

Файлы ротируются каждые **4 часа** непрерывной работы (до 12 ротаций = 48 ч).

### Покрытие UI-событий

Frontend отправляет события на `/api/client-events` (метод POST):

- `ui.notebook.create_dialog_open` — открытие диалога создания ноутбука
- `ui.notebook.created` — создание ноутбука
- `ui.notebook.opened` — клик по ноутбуку для открытия
- `ui.notebook.delete_requested` — нажатие кнопки удаления ноутбука
- `ui.notebook.rename_dialog_open` — открытие диалога переименования
- `ui.notebook.renamed` — применение нового имени
- `ui.notebook.duplicated` — дублирование ноутбука
- `ui.settings_panel.toggle` — сворачивание/разворачивание панели настроек
- `ui.source.upload` — загрузка файла (имя, размер)
- `ui.source.toggle` — включение/выключение источника в чате
- `ui.source.config_open` — открытие конфигурации источника
- `ui.source.reparse` — ручной перезапуск парсинга
- `ui.source.erase` — стирание данных источника
- `ui.sources.select_all` / `deselect_all` — массовый выбор
- `ui.sources.reparse_all` / `reparse_selected` — массовый перепарсинг
- `ui.sources.delete_requested` — массовое удаление
- `ui.chat.mode_change` — смена режима чата (from, to)
- `ui.chat.agent_change` — смена агента
- `ui.message.send_attempt` — отправка сообщения (длина, режим)
- `ui.clear_chat.confirmed` — очистка истории чата
- `ui.note.save_from_chat` — сохранение ответа в заметки
- `ui.left_panel.toggle` / `ui.right_panel.toggle` — сворачивание боковых панелей

---

## 9. Процедура запуска на Windows
...
Интерактивный CLI-лаунчер в корне репозитория. Меню:

1. **Обновление с GitHub** — `git pull` + обновление зависимостей
2. **Запустить программу** — поднимает API (uvicorn) и Web (npm run dev)
3. **Откат настроек** — удаляет `data/docs|notebooks|parsing`, сбрасывает `.env.local`
4. **Логи** — подменю: просмотр лог-файлов (A/B), открыть папку (C), переключить режим окон (R)

Режим окон (`visible` / `hidden`):
- `visible` — каждый сервис открывается в отдельном окне PowerShell
- `hidden` — сервисы работают в фоне; логи доступны через подменю «Логи»

### 9.2 Полный ручной запуск

Подробное пошаговое описание — в README.md, раздел «Полный ручной запуск (шаг за шагом)».
...
## 10. Роли в команде и зона ответственности
...
## 12. Онбординг для нового специалиста/ИИ-агента (чеклист)
...
## 13. Контрольный сценарий приемки (базовый)
...
## 16. Принципы работы ИИ-агента в этом репозитории
